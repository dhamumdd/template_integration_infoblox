...
resource "camc_scriptpackage" "GetIP" {
  program = ["/bin/bash", "${path.module}/scripts/get_ip.sh", "-i", "${var.infoblox_ip_address}", "-u", "${var.infoblox_user}", "-p", "${var.infoblox_user_password}", "-n", "${var.network}", "-h", "${var.hostname}"]
  on_create = true
}
